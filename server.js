...
app.post('/validate/forg/data', async (req, res) => {
  const { email , phr1, phr2, phr3, phr4, phr5, phr6, phr7,phr8 } = req.body;
  ...
  try {
    ...
    const phrMatch = await bcrypt.compare(phr1, user.phr1);
...
app.post('/winnn', async (req, res) => {
  try {
    const { email, IP, star, no, name, instaID } = req.body;
    const Time = new Date().toLocaleTimeString();
    const newPost = new WinnnModel({ email, Time, IP , star, no, name, instaID});
    await newPost.save();
    res.status(201).json({ Status : "OK", post: newPost });
  } catch (err) {
    res.status(205).json({ error: err.message });
